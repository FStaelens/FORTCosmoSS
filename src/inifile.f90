...
!!$  subroutine assign_output(var,var_array)
!!$    implicit none
!!$    character(len=*), intent(out) :: var
!!$    character(len=*), allocatable, dimension(:), intent(inout) :: var_array
!!$    integer :: i
...
!!$
!!$    do i=1, size(input)
!!$       if(input(i)%name=='output') then
!!$          allocate(var_array(size(input(i)%output_list)))
!!$          var_array = input(i)%output_list 
!!$          output_exists = .true.
!!$       end if
!!$    enddo
!!$    if(output_exists.eqv..false.)then
!!$       allocate(var_array(0))
!!$       print*, 'Warning : no output list found in "input.ini"'
!!$    endif
!!$    var = ''
!!$    if(size(var_array)==1) then
!!$       var = var_array(1)
...
!!$  subroutine assign(var,var_array,name)
!!$    implicit none
!!$    real(kind=8), intent(out) :: var
!!$    real(kind=8), dimension(:), intent(inout) :: var_array
!!$    character(len=*), intent(in) :: name
!!$    integer :: i
!!$    logical :: flag
!!$
...
!!$       end if
!!$    enddo
!!$    if(flag.eqv..false.) print*, 'Warning : no matching of label "', trim(adjustl(name)), '" found in input.ini'
!!$    
!!$    var = 0d0
!!$    if(size(var_array)==1) then
!!$       var = var_array(1)
